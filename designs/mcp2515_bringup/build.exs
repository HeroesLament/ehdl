#!/usr/bin/env elixir
# designs/mcp2515_bringup/build.exs
#
# Usage:
#   mix run designs/mcp2515_bringup/build.exs               # build + load to SRAM (volatile)
#   mix run designs/mcp2515_bringup/build.exs --flash       # build + write SPI flash (persistent)
#   mix run designs/mcp2515_bringup/build.exs --build-only  # build the .bit only, don't touch the board
#
# Requires: yosys, nextpnr-ecp5, ecppack, fujprog on PATH.
# Reuses the stock ULX3S board LPF — the top-level port names (clk_25mhz,
# ftdi_rxd, led, gp0..gp3) match its COMP names, so no design-specific pin file.

defmodule Mcp2515Bringup.Build do
  @build_dir "designs/mcp2515_bringup/build"
  @top_module Mcp2515Bringup.Top
  @modules [Mcp2515Bringup.Top]
  @lpf_file "lib/hw/boards/ulx3s/ulx3s_v20_nextpnr.lpf"
  @top_name "mcp2515_bringup"
...
Mcp2515Bringup.Build.run(System.argv())
