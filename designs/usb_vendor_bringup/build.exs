#!/usr/bin/env elixir
# designs/usb_vendor_bringup/build.exs
#
# Usage:
#   mix run designs/usb_vendor_bringup/build.exs
#   mix run designs/usb_vendor_bringup/build.exs --flash       # build + write SPI flash (persistent)
#   mix run designs/usb_vendor_bringup/build.exs --build-only  # build the .bit only, don't touch the board
#
# Requires:
#   yosys, nextpnr-ecp5, ecppack, fujprog on PATH

defmodule UsbVendorBringup.Build do
  @build_dir "designs/usb_vendor_bringup/build"
  @top_module UsbVendorBringup.Top
...
UsbVendorBringup.Build.run(System.argv())
