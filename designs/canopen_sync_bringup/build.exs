#!/usr/bin/env elixir
# designs/canopen_sync_bringup/build.exs
#
# Usage:
#   mix run designs/canopen_sync_bringup/build.exs               # build + load to SRAM (volatile)
#   mix run designs/canopen_sync_bringup/build.exs --flash       # build + write SPI flash (persistent)
#   mix run designs/canopen_sync_bringup/build.exs --build-only  # build the .bit only, don't touch the board
#
# Requires: yosys, nextpnr-ecp5, ecppack, fujprog on PATH.
# Reuses the stock ULX3S board LPF — the top-level port names (clk_25mhz,
# ftdi_rxd, led, gn13/gp13/gn12/gp12) match its COMP names, so no design-specific pin file.

defmodule CanopenSyncBringup.Build do
  @build_dir "designs/canopen_sync_bringup/build"
  @top_module CanopenSyncBringup.Top
  @modules [CanopenSyncBringup.Top]
  @lpf_file "lib/hw/boards/ulx3s/ulx3s_v20_nextpnr.lpf"
  @top_name "canopen_sync_bringup"
...
CanopenSyncBringup.Build.run(System.argv())
