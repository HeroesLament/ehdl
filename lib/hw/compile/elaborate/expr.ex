defmodule Hw.Compile.Elaborate.Expr do
      ...
      %Ops.Mem{sync_read: sync_read} = mem ->
        ...
        clock = case sync_read do
          ...
          clock_name when is_atom(clock_name) ->
            # Find the clock in the design
            Enum.find(d1.clocks, &(&1.name == clock_name))
        end
  ...
  def build_expr({:ternary, cond, then_val, else_val}, signal_map, instance_map, memory_map, design) do
    ...
    # Infer width from the two branches
    width = infer_width(then_result, else_result)
    signed = infer_signedness(then_result, else_result)

    # Match constant widths
    then_result = match_value_to_width(then_result, width, signed)
    else_result = match_value_to_width(else_result, width, signed)
  ...
  def infer_width(%Signal{width: w}, _), do: w
  def infer_width(_, %Signal{width: w}), do: w
  def infer_width(%Const{width: w}, _), do: w
  def infer_width(_, %Const{width: w}), do: w
  def infer_width(_, _), do: 32
  ...
  def match_const_widths(%Const{} = c, %Signal{} = s) do
    {%Const{c | width: s.width, signed: s.signed}, s}
  end
  def match_const_widths(%Signal{} = s, %Const{} = c) do
    {s, %Const{c | width: s.width, signed: s.signed}}
  end
